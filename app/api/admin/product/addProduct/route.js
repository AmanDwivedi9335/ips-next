...
export async function POST(request) {
  ...
  try {
    ...
    if (!title || !description || !category || !productFamily) {
    ...
    try {
      ...
      if (featuresString) {
        ...
        features = Array.isArray(parsedFeatures)
          ? parsedFeatures.filter((f) => f.title && f.description)
          : [];
      }
    } catch (error) {
    ...
    // Filter language images and collect URLs and languages
    ...
    const basePrice =
      pricing.find((p) => typeof p?.price === "number" && !isNaN(p.price))?.price || 0;
